...
func TestPool_QueryFlushVote(t *testing.T) {
	...
	// cannot find
	result, err = votePool.GetVotesByEventHash(ToBscCrossChainEvent, vote1.EventHash)
	require.NoError(t, err)
...
func TestPool_ValidatorSetUpdate(t *testing.T) {
	...
	// add validator 1
	addVal := types.Validator{PubKey: val1.PubKey, Address: val1.Address, BlsPubKey: val1.BlsPubKey, VotingPower: 10}
	validatorUpdates = []*types.Validator{
		&addVal,
	}
	validatorUpdateEvents = types.EventDataValidatorSetUpdates{
